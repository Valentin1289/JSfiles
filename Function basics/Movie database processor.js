...
const movies = [
  ...
  function dramas(vect){
    let arr=[];
    for (const x of vect) {
        for (const y of x.genres) {
            if(y=="Drama") {arr.push(x.title);}
        }
    }
    return arr;
  }
  console.log(dramas(movies));
 function findByGenre(gn,vect){
  let arr=[];
  for (const x of vect) {
    for (const y of x.genres) {
      if(y===gn) arr.push(x.title);
    }
  }
  return arr;
 }
  console.log(findByGenre("Comedy",movies));
 function longestMovie(vect){
  let max=0,poz;
  for (const x of vect) {
   if(max<x.runtime){max=x.runtime;poz=x;}
  }
return poz;
 }
 console.log(longestMovie(movies));
 function longestMovieByGenre(gen,vect){
  let max=0,poz=vect[0];
  for (const x of vect) {
    for (const y of x.genres) {
     if(y===gen && max<x.runtime)poz=x;
    }
  }
return poz;
}
console.log(longestMovieByGenre("Comedy",movies));
